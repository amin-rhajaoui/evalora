...
export default function Results() {
  ...
  const displayFeedback = feedback || {
    ...
    improvements: [],
    advice: [],
    detailed_scores: { monologue: [], debat: [], general: [] },
    monologue_duration: "--:--",
    debat_duration: "--:--",
    total_duration: "--:--",
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 py-8 px-4 print:bg-white print:py-2">
      <div className="max-w-3xl mx-auto space-y-6">
        {/* Header */}
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-amber-400 to-orange-500 rounded-3xl mb-4 shadow-lg print:shadow-none">
            <Trophy className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-4xl font-bold text-slate-800 mb-2">Vos resultats</h1>
          {selectedAvatar && (
            <p className="text-slate-600 mt-1">
              Examinateur: <span className="font-semibold text-slate-800">{selectedAvatar.name}</span>
            </p>
          )}
          <Card className="mt-4 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 max-w-xl mx-auto print:border">
            <div className="p-4 text-center">
              <p className="text-sm text-slate-700">
                ...
                <span className="block mt-1 text-xs text-slate-600">Chaque entrainement est une etape vers le progres</span>
              </p>
            </div>
          </Card>
        </div>
        ...
        {/* Score Card */}
        <Card className="overflow-hidden border-2 border-slate-200 shadow-lg print:shadow-none">
          <div className={cn(
        ...
        {/* Detailed Scores - Monologue */}
        {displayFeedback.detailed_scores?.monologue?.length > 0 && (
          <DetailedScoreSection
            title="Monologue - Scores detailles"
            icon={<Target className="w-5 h-5 text-green-600" />}
            scores={displayFeedback.detailed_scores.monologue}
            expanded={expandedSections["monologue"] ?? false}
            onToggle={() => toggleSection("monologue")}
            colorClass="green"
          />
        )}

        {/* Detailed Scores - Debat */}
        {displayFeedback.detailed_scores?.debat?.length > 0 && (
          <DetailedScoreSection
            title="Debat - Scores detailles"
            icon={<TrendingUp className="w-5 h-5 text-purple-600" />}
            scores={displayFeedback.detailed_scores.debat}
            expanded={expandedSections["debat"] ?? false}
            onToggle={() => toggleSection("debat")}
            colorClass="purple"
          />
        )}

        {/* Detailed Scores - General */}
        {displayFeedback.detailed_scores?.general?.length > 0 && (
          <DetailedScoreSection
            title="Competences generales - Scores detailles"
            icon={<Star className="w-5 h-5 text-blue-600" />}
            scores={displayFeedback.detailed_scores.general}
            expanded={expandedSections["general"] ?? false}
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center p-5 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border-2 border-purple-200">
                ...
                <p className="text-sm text-slate-600 font-medium">Debat</p>
              </div>
        ...
        {conversationTranscript && conversationTranscript.length > 0 && (
          <Card className="border-2 border-indigo-200 shadow-sm bg-gradient-to-br from-indigo-50 to-violet-50">
            ...
            <CardContent className="pt-6">
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2 print:max-h-none print:overflow-visible">
          ...
          <Card className="bg-gradient-to-r from-sky-50 to-blue-50 border-2 border-sky-200 max-w-md">
            <div className="p-4 text-center">
              <p className="text-sm text-slate-700">
                ...
                Chaque entrainement vous rapproche de votre objectif. Continuez ainsi !
              </p>
            </div>
          </Card>
        </div>
...
const CRITERION_LABELS: Record<string, string> = {
  presentation: "Presentation du document",
  description: "Description",
  analyse_opinion: "Analyse et opinion",
  coherence: "Coherence",
  aisance: "Aisance",
...
function DetailedScoreSection({
  ...
  const headerBg = {
    ...
    blue: "bg-gradient-to-r from-blue-50 to-sky-50",
  }[colorClass]
  ...
  return (
    <Card className={cn("border-2 shadow-sm", borderColor)}>
      <button
        onClick={onToggle}
        className={cn(
          ...
          headerBg,
          "border-b",
          expanded ? borderColor : "border-transparent",
          "rounded-t-lg"
        )}
      >
        ...
        <CardContent className="pt-4 pb-4">
          <div className="space-y-4">
            {scores.map((criterion) => {
              const percent = criterion.max_score > 0 ? (criterion.score / criterion.max_score) * 100 : 0
              return (
                <div key={criterion.criterion} className="space-y-1.5">
